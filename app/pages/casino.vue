<template>
  <div class="h-full flex flex-col gap-4 overflow-y-auto">
    ...
    <div class="flex items-center justify-between sticky top-0 bg-transparent z-10 pb-2">
      <h1 class="text-2xl font-bold text-white drop-shadow-lg">คาสิโนสด</h1>
      <div class="relative">
        <input 
          v-model="searchQuery"
          type="text" 
...
const { data, pending, error } = await useFetch<CasinoGame[]>(`${baseUrl}/api/v1/lobby/casino`)
...
watchEffect(() => {
  ...
  if (error.value) {
    console.error('Error fetching casino games:', error.value)
  }
})

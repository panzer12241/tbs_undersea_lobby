<template>
  <div class="h-full flex flex-col gap-4 overflow-y-auto">
    ...
    <div class="flex items-center justify-between sticky top-0 bg-transparent z-10 pb-2">
      <h1 class="text-2xl font-bold text-white drop-shadow-lg">เกมโต๊ะ</h1>
      <div class="relative">
        <input 
          v-model="searchQuery"
          type="text" 
...
const games = ref<CardGame[]>([])
const loading = ref(true)
const searchQuery = ref('')

// Fetch games on mount
const { data, pending, error } = await useFetch<CardGame[]>(`${baseUrl}/api/v1/lobby/card`)
...
watchEffect(() => {
  ...
  if (error.value) {
    console.error('Error fetching card games:', error.value)
  }
})
